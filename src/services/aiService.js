...
// Groq SDK menggunakan interface yang kompatibel dengan OpenAI
const groq = new Groq({ apiKey: process.env.GROQ_API_KEY });

// ── Model yang tersedia di Groq (gratis & cepat!) ─────────────────────────────
// llama-3.3-70b-versatile  → Terbaik, pintar & cepat (RECOMMENDED)
// llama-3.1-8b-instant     → Paling cepat, ringan
// llama3-70b-8192          → Konteks panjang 8192 token
// mixtral-8x7b-32768       → Konteks sangat panjang 32768 token
...
// ── Main AI Call Function (Groq) ───────────────────────────────────────────────
async function callAI({ systemPrompt, messages, temperature = 0.8, maxTokens = 1024 }) {
  try {
    // Groq membatasi temperature max 2.0, beberapa model max 1.0
    // Kita clamp ke 1.0 untuk kompatibilitas semua model
    ...
    if (err.status === 401) return '❌ Groq API Key tidak valid! Cek konfigurasi `.env`.';
    if (err.status === 429) {
      const retryAfter = err.headers?.['retry-after'] ?? '30';
      return `⏳ Rate limit Groq tercapai! Coba lagi dalam **${retryAfter} detik**.\n> 💡 Tips: Groq gratis punya limit ~30 req/menit.`;
    }
    if (err.status === 400) return `❌ Request tidak valid: ${err.message}`;
    if (err.status === 503) return '🔧 Server Groq sedang sibuk. Coba lagi sebentar!';
    if (err.status === 413) return '📝 Pesan terlalu panjang! Coba pertanyaan yang lebih singkat.';

    return `❌ Groq Error (${err.status ?? 'unknown'}): ${err.message}`;
  }
}
